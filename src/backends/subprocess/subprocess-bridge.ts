  ...
  return (pi: ExtensionAPI): void => {
    ...
    pi.on("context", (event) => {
      ...
      if (markerIndex === -1) {
        if (!markerObserved) {
          throw new Error(
            "Subagent runtime subprocess marker was absent before the first provider request.",
          );
        }
        return;
      }
    ...
    pi.on("tool_call", (event) => {
      if (!input.effectiveToolNames.includes(event.toolName)) {
        return {
          block: true,
          reason: `Tool ${event.toolName} is outside the approved subagent runtime subprocess allowlist.`,
        };
      }
    });
